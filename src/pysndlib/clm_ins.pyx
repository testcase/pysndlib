...
def fm_trumpet(startime, dur, frq1=250, frq2=1500, amp1=.5, amp2=.1, 
    ...
    mod1_f = clm.make_env(stretch_envelope(indenv1, 25, ampattpt1, 75, dec_01), scaler=(modfrq1 * (modind12 - modind11)), duration=dur)
    mod1 = clm.make_oscil(0.0)
    car1 = clm.make_oscil(0.0)
    car1_f = clm.make_env(stretch_envelope(ampenv1, 25, ampattpt1, 75, ampdecpt1), scaler=amp1, duration=dur)
    
    mod2_f = clm.make_env(stretch_envelope(indenv2, 25, ampattpt2, 75, dec_01), scaler=(modfrq2 * (modind22 - modind21)), duration=dur)

    mod2 = clm.make_oscil(0.0)
    car2 = clm.make_oscil(0.0)
    car2_f = clm.make_env(stretch_envelope(ampenv2, 25, ampattpt2, 75, ampdecpt2), scaler=amp2, duration=dur)
...
def gong(start_time, duration, frequency, amplitude, degree=0.0, distance=1.0, reverb_amount=.005):
    ...
    ampfun = clm.make_env(stretch_envelope(expf, atpt, atdur, None, None), scaler=amplitude, duration=duration)
...
def reson(startime, dur, pitch, amp, numformants, indxfun, skewfun, pcskew, skewat, skewdc,
    ...
    for k in range(0, numformants):
        ...
        if indxdc == 0:
            indxdc = 75
        indfs[k] = clm.make_env(stretch_envelope(indxfun, 25, indxat, 75, indxdc), duration=dur, scaler=(dev1 - dev0), offset=dev0)
        ampfs[k] = clm.make_env(stretch_envelope(ampf, 25, ampat, 75, ampdc), duration=dur, scaler=(rsamp * amp * rfamp) / totalamp)
        c_rats[k] = harm
...
def cellon(beg, dur, pitch0, amp, ampfun, betafun, beta0, beta1, betaat, betadc, ampat, ampdc, dis, pcrev, deg,
    ...
    pvibenv = clm.make_env(stretch_envelope(pvibfun, 25, pvbap, 75, pvbdp), duration=dur, scaler=clm.hz2radians(pvibpc * pitch0))
    rvibenv = clm.make_env(rvibfun, duration=dur, scaler=clm.hz2radians(rvibpc * pitch0))
    glisenv = clm.make_env(stretch_envelope(pvibfun, 25, pvbap, 75, pvbdp), duration=dur, scaler=clm.hz2radians(pvibpc * pitch0))
    amplenv = clm.make_env(stretch_envelope(ampfun, 25, ampap, 75, ampdp), scaler=amp, duration=dur)
    betaenv = clm.make_env(stretch_envelope(betafun, 25, betap, 75, betdp), duration=dur, scaler=(beta1 - beta0), offset=beta0)
...
def make_piano_ampfun(dur, piano_attack_duration):
    db_drop_per_second = -10.0
    releaseAmp = clm.db2linear(db_drop_per_second * dur)
...
def lbj_piano(startime, duration, frequency, amplitude, pfreq=None, degree=45, reverb_amount=0, distance=1):
    ...
    if not clm.is_number(pfreq):
        pfreq = frequency
        
    partials = clm.normalize_partials(get_piano_partials(pfreq))
    beg: cython.long = clm.seconds2samples(startime)
    newdur: cython.long = duration + piano_attack_duration + piano_release_duration
    end: cython.long = beg + clm.seconds2samples(newdur)
    env1dur: cython.double = newdur - piano_release_duration
    siz = len(partials) // 2
    env1samples: cython.long = beg + clm.seconds2samples(env1dur)
    freqs: np.ndarray  = np.zeros(siz, dtype=np.double)
    phases: np.ndarray  = np.zeros(siz, dtype=np.double)
    alist: np.ndarray  = np.zeros(siz, dtype=np.double)
    locs = clm.make_locsig(degree, distance, reverb_amount)
    ampfun1 = make_piano_ampfun(env1dur, piano_attack_duration)
    ampenv1 = clm.make_env(ampfun1, scaler=amplitude, duration=env1dur, base=10000.0)
    ampenv2 = clm.make_env([0,1,100,0], scaler=amplitude * ampfun1[len(ampfun1)-1], duration=env1dur, base=1.0)
    
    
    i: cython.long = 0
    
    for i in range(siz):
        freqs[i] =  clm.hz2radians(partials[::2][i]*frequency)
        alist[i] =  partials[1::2][i]
        
    obank: clm.mus_any = clm.make_oscil_bank(freqs, phases, alist, stable=True)
    
    for i in range(beg,env1samples):
        clm.locsig(locs, i, clm.env(ampenv1) * clm.oscil_bank(obank))
    for i in range(env1samples, end):
        clm.locsig(locs, i, clm.env(ampenv2) * clm.oscil_bank(obank))
...
def clm_expsrc(beg, dur, input_file, exp_ratio, src_ratio, amp: cython.double, rev=False, start_in_file=False):
    stf = math.floor((start_in_file or 0)*clm.clm_srate(input_file))
    two_chans = clm.clm_channels(input_file) == 2 and clm.clm_channels(clm.default.output) == 2
    ...
    srcA = clm.make_src(lambda g, d : clm.granulate(exA), srate=src_ratio)
    
    rev_amp: cython.double = 0
    
    if two_chans:
        srcB: clm.mus_any = clm.make_src(lambda g, d : clm.granulate(exB),srate=src_ratio)
...
def exp_snd(beg, dur, file, amp, exp_amt=1.0, ramp=.4, seglen=.15, sr=1.0, hop=.05, ampenv=[0,0,.5,1,1,0]):
    ...
    max_seg_len = max_envelope(seglen) if is_pair(seglen) else seglen
